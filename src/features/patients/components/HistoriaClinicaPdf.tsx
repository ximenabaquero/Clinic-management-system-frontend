...
const CATEGORIES: { title: string; fields: FindingKey[] }[] = [
  {
    ...
    fields: ["diabetes", "cardiac", "varicose_veins", "thrombosis", "fluid_retention", "surgeries", "hepatitis", "kidney_issues", "thyroid_issues"],
  },
  {
    title: "Digestivos",
    fields: ["constipation", "gastritis", "irritable_bowel", "reflux", "intestinal_habit"],
  },
  {
    title: "Condiciones de piel",
    fields: ["lupus_notes", "allergy_notes", "keloid_notes", "vitiligo_notes", "dermatitis_notes", "other_skin_notes"],
  },
  {
...
function categoryHasData(fields: FindingKey[], findings: FullRecord["clinical_findings"]): boolean {
...
function FieldRow({ label, value, active }: { label: string; value: string; active: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: "4px" }}>
      <span style={{ fontSize: "10px", color: active ? "#374151" : "#9ca3af", flex: 1, paddingRight: "8px" }}>
        {label}
      </span>
      <span style={{ fontSize: "10px", fontWeight: active ? 700 : 400, color: active ? "#b45309" : "#9ca3af", whiteSpace: "nowrap" }}>
        {value}
      </span>
    </div>
  );
...
function CategoryBox({ title, fields, findings }: {
  title: string;
  fields: FindingKey[];
  ...
  return (
    <div style={{
      ...
      {hasData ? activeFields.map((field) => {
        const raw = findings[field];
        ...
        <p style={{ fontSize: "10px", color: "#9ca3af", fontStyle: "italic" }}>Ninguna</p>
      )}
    </div>
  );
}
...
const HistoriaClinicaPdf = forwardRef<HTMLDivElement, Props>(
  ({ record, evaluationId, currentYear }, ref) => {
    const { evaluation, snapshot, procedures, clinical_findings, lab_results, confirmed_by } = record;

    const firstProcedureDate = procedures[0]?.created_at;
    const labEntries = Object.entries(lab_results) as [keyof typeof lab_results, unknown][];
      ...
      <div className="fixed -left-[9999px] top-0" aria-hidden="true">
        ...
        >
          ...
          <div style={{ padding: "36px 48px 48px" }}>
            ...
            <div style={{ borderTop: "2px solid #111827", marginBottom: "20px" }} />
              ...
              <div style={{ flex: "0 0 58%" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 16px" }}>
                  {[
                    ...
                    { label: "Fecha de nacimiento", value: new Date(snapshot.date_of_birth).toLocaleDateString("es-CO", { timeZone: "UTC" }) },
                    { label: "Edad al momento", value: `${evaluation.patient_age_at_evaluation} años` },
                    { label: "Sexo biológico", value: snapshot.biological_sex },
                    { label: "Estado civil", value: MARITAL_STATUS_LABELS[snapshot.marital_status] ?? snapshot.marital_status },
            ...
            <div style={{ marginBottom: "20px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                ...
                <div style={{
                  background: "#fff",
                  border: "1px solid #e5e7eb",
                  borderLeft: hasLabData ? "3px solid #d97706" : "3px solid #e5e7eb",
                  borderRadius: "6px",
                  padding: "10px 12px",
                }}>
                  <p style={s.categoryTitle}>Resultados de laboratorio</p>
                  {(() => {
                    const activeLabEntries = labEntries.filter(([, val]) =>
            ...
            {procedures.map((proc, idx) => (
              <div key={proc.id} style={{ marginBottom: "20px" }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px", marginBottom: "10px" }}>
                  <thead>
                    <tr style={{ background: "#f0fdfa" }}>
                      <th style={{ textAlign: "left", padding: "8px 12px", fontSize: "9px", textTransform: "uppercase", color: "#0d9488", fontWeight: 700 }}>
                        Procedimiento
                      </th>
                      <th style={{ textAlign: "right", padding: "8px 12px", fontSize: "9px", textTransform: "uppercase", color: "#0d9488", fontWeight: 700 }}>
                        Precio
                      </th>
                    </tr>
                  </thead>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: "10px" }}>
                  <div style={{ borderTop: "2px solid #374151", paddingTop: "6px", textAlign: "right" }}>
                    <p style={{ fontSize: "9px", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "2px" }}>
                      Total sesión
                    </p>
                ...
                {proc.notes && (
                  <div>
                    ...
                    <div style={{ border: "1px solid #e5e7eb", borderRadius: "6px", padding: "10px", fontSize: "12px", color: "#374151", fontStyle: "italic", lineHeight: 1.6 }}>
                      {proc.notes}
                    </div>
                  </div>
                )}
